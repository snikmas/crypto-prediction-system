...
def save_df(df):
    ...
    logger.info(f"file_size: {file_size:1f} KB")
...
def load_data_from_db(symbol: str, limit: int = None) -> pd.DataFrame:
    ...
    try:
        ...
        logger.error(f"Error during load_data_from_db: {e}")
        return pd.DataFrame()
    finally:
        conn.close()
...
def calculate_volatility(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['volatility_lag_24h'] = df['volatility_24h'].shift(24)
    return df
...
def create_targets(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['target_vol_regime'] = (hourly_volatility.shift(-1) > vol_threshold).astype(int)
